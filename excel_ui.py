...
def same_month_last_year(data, year, month, business, region, kind):
    actual = actual_value(data, year - 1, month, business, region, kind)
    if actual is not None:
        return actual
    for stage in ["마감", "3차", "2차", "1차"]:
        snap = snapshot(data, year - 1, month, stage, business, region, kind)
        if snap["has"]:
            return snap["forecast"] or 0
    return None
...
def row_metrics(data, year, month, business, region, kind):
    ...
    current_compare = best_month(data, year, month, business, region, kind)
    prev_same = same_month_last_year(data, year, month, business, region, kind)
    yoy_diff = current_compare - prev_same if prev_same is not None else None
    yoy_rate = (yoy_diff / prev_same) if prev_same not in (None, 0) else None
    ...
    return {
        ...
        "avg": avg,
        "prev_same": prev_same,
        "compare_current": current_compare,
        "yoy_diff": yoy_diff,
        "yoy_rate": yoy_rate,
...
def sum_rows(rows, label, business=None):
    ...
    prev_same = sum_nullable("prev_same")
    compare_current = sum_nullable("compare_current")
    yoy_diff = compare_current - prev_same if compare_current is not None and prev_same is not None else None
    yoy_rate = (yoy_diff / prev_same) if prev_same not in (None, 0) else None

    return {
        "is_total": True,
        "is_grand": label.endswith("전체"),
        "label": label,
        "business": business,
        "region": "[소계]",
        "kind": "",
        ...
        "ytd": ytd,
        "avg": sum(r.get("avg", 0) for r in rows),
        "prev_same": prev_same,
        "compare_current": compare_current,
        "yoy_diff": yoy_diff,
        "yoy_rate": yoy_rate,
        "prev_first": sum_nullable("prev_first"),
        "prev_preclose": sum_nullable("prev_preclose"),
        "prev_close": sum_nullable("prev_close"),
        "first": sum_nullable("first"),
        "second": sum_nullable("second"),
        "third_confirmed": sum_nullable("third_confirmed"),
        "third_forecast": sum_nullable("third_forecast"),
        "close": sum_nullable("close"),
        "close_has": all(r.get("close_has") for r in rows),
        "next_first": sum_nullable("next_first"),
        "carryover": sum(r.get("carryover", 0) for r in rows),
        "qproj": sum(r.get("qproj", 0) for r in rows),
        ...
        "q4proj": sum(r.get("q4proj", 0) for r in rows),
        "second_half": sum(r.get("second_half", 0) for r in rows),
    }
...
def render_report(report, user, capture=False):
    ...
    for r in report["rows"]:
        ...
        yoy_class = "positive" if (r.get("yoy_rate") or 0) > 0 else ("negative" if (r.get("yoy_rate") or 0) < 0 else "")
        vals = (
            f"<td>{money_m(r.get('ytd'))}</td>"
            f"<td>{money_m(r.get('avg'))}</td>"
            f"{hist}"
            f"<td class='yoy'>{money_m(r.get('prev_same'))}</td>"
            f"<td class='yoy {yoy_class}'>{pct_delta(r.get('yoy_rate'))}</td>"
            f"<td>{money_m(r.get('prev_first'))}</td>"
            f"<td>{money_m(r.get('prev_preclose'))}</td>"
            f"<td>{money_m(r.get('prev_close'))}</td>"
            f"<td class='c1'>{money_m(r.get('first'))}</td>"
            f"<td class='c2'>{money_m(r.get('second'))}</td>"
            f"<td class='c3c'>{money_m(r.get('third_confirmed'))}</td>"
            f"<td class='c3f'>{money_m(r.get('third_forecast'))}</td>"
            f"<td class='cc'>{money_m(r.get('close'))}</td>"
            f"<td>{money_m(r.get('next_first'))}</td>"
            f"<td>{money_m(r.get('qproj'))}</td>"
            f"<td>{money_m(r.get('october'))}</td>"
            f"<td>{money_m(r.get('november'))}</td>"
            f"<td>{money_m(r.get('december'))}</td>"
            f"<td>{money_m(r.get('q4proj'))}</td>"
            f"<td>{money_m(r.get('second_half'))}</td>"
        )
...
def excel_health():
    ...
    return {
        "status": "ok",
        "runtime": "excel-layout-v2-yoy",
        "initialized": bool(data.get("meta", {}).get("initialized")),
        "users": len(data.get("users", [])),
        "entries": len(data.get("entries", [])),
        "actuals": len(data.get("actuals", {})),
        "seed_available": bool(base.os.environ.get("AUTO_SEED_GZ", "").strip()),
    }
